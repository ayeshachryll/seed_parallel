...
int main()
{
    ...
    pthread_t accept;
    pthread_create(&accept, nullptr, accept_thread, &listen_fd);
    ...
    while (1)
    {
        ...
        else if (choice == 2)
        {
            ...
            else
            {
                ...
                // Find all available ports for this file
                ...
                // Distribute chunks among available ports
                std::vector<std::vector<ChunkInfo>> port_chunks(num_ports);

                int current_port = 0; // Start with first port
                for (int i = 0; i < num_chunks; i++)
                {
                    ...
                    // Calculate chunk size without std::min
                    ...
                    // Move to next port (simple round-robin)
                    current_port++;
                    if (current_port >= num_ports)
                    {
                        current_port = 0; // Reset to first port
                    }
                }

                // Create download threads for each port
                ...
                for (int i = 0; i < num_ports; i++)
                {
                    ...
                    pthread_t thread;
                    pthread_create(&thread, nullptr, download_from_specific_port, port_info);
                    download_threads.push_back(thread);

                    // std::cout << "Port " << available_ports[i] << " assigned " << port_chunks[i].size() << " chunks\n";
                }
                ...
                // std::cout << "Download threads started. Use option 3 to check progress.\n";
            }
        }
